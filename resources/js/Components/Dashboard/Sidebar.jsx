...
export default function Sidebar(props) {
    const {auth:{role,permissions}} = props
    

    function isViewMenu(item)
    {
        return role == 'super' || permissions.includes(item);
    }
            ...
            <div className="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
                <ul className="navbar-nav">
                    ...
                    <li className="nav-item mt-3">
                        <h6 className="ps-4 ms-2 text-uppercase text-xs font-weight-bolder opacity-6">Gerenciamento do Sistema</h6>
                    </li> 
                    {
                        isViewMenu('routes.index')  && (
                            <>
                                <li className="nav-item">
                                    <Link className={`${route().current('routes.*') && 'active'} nav-link`} href={route('routes.index')}>
                                        <div className="icon icon-shape icon-sm border-radius-md text-center me-2 d-flex align-items-center justify-content-center">
                                            <i className="ni ni-single-02 text-dark text-sm opacity-10" />
                                        </div>
                                        <span className="nav-link-text ms-1">Rotas do Sistema</span>
                                    </Link>
                                </li>
                                ...
                                <li className="nav-item">
                                    <Link className={`${route().current('roles.*') && 'active'} nav-link`} href={route('roles.index')}>
                                        ...
                                        <span className="nav-link-text ms-1">Roles do Sistema</span>
                                    </Link>
                                </li>
                            </>
                        )
                    ...
                    {/* <li className="nav-item">
                        ...
                        className={`${route().current('users.*') ? 'visible' : 'invisible'}`}
                        >
                        <li className="nav-item">
                        <Link className={`${route().current('users.*') && 'active'} nav-link`} href={route('users.index')}>
                            <div className="icon icon-shape icon-sm border-radius-md text-center me-2 d-flex align-items-center justify-content-center">
