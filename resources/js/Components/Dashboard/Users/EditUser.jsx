...
export default function EditUser({close, model}) {
    ...
    const {data, setData, put, reset, errors} = useForm({ name: model.name, email: model.email, username: model.username, address: model.address, password: model.password, role: model?.role });
    ...
    useEffect(() => {
        setData({...data,
            name: model.name, email: model.email, username: model.username, address: model.address, password: model.password, role: model?.role
        });
    }, [model]);
        ...
        <>
            <form onSubmit={onSubmit}>
                <div className="modal-body">
                        ...
                        <div className="form-group">
                            <label htmlFor="role" className="col-form-label">Perfil:</label>
                            <select name="role" id="role" value={data.role} onChange={onChange} className='form-control'  >
                                <option value="">--SELECIONE--</option>
                                {
                                    roles.map( resp =>  <option key={resp.id} value={resp.id}>{resp.name}</option>)
                                }
                            </select>
                            {errors && <div className='text-danger mt-1'>{errors.role}</div>}
                        </div>
